...
void GenerateProto(const google::protobuf::Descriptor* descriptor,
  ...
  for (int i = 0; i < descriptor->field_count(); ++i) {
    ...
    if (field->type() == google::protobuf::FieldDescriptor::TYPE_MESSAGE) {
      type_name = field->message_type()->full_name();  // 嵌套消息的全名
    } else {
      type_name = field->type_name();  // 基础类型名（如 "int32"）
    }
...
void MessageCallback(const std::shared_ptr<RawMessage>& raw_msg) {
  ...
  const std::string type_name =
      "apollo.common.StatusPb";  //"davinci_adas.apa.odometry.Odometry";
  if (type_name.empty()) {
    AERROR << "Failed to get message type name";
    return;
  }
...
int main(int argc, char* argv[]) {
  ...
  auto reader =
      node->CreateReader<RawMessage>("/davinci/apa/odometry", MessageCallback);
